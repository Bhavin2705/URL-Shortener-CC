...
export default function UrlForm({ onSubmit, loading }: Props) {
  ...
  const handle = async (e: FormEvent) => {
    e.preventDefault();
    if (!url.trim()) return;
    if (expires && new Date(expires).getTime() <= Date.now()) return;
      ...
      {adv && (
        <div className="grid sm:grid-cols-3 gap-4 pt-2 border-t border-surface-800">
          <div>
            ...
            <input type="text" value={title} onChange={e => setTitle(e.target.value)} placeholder="My campaign link" className="input" maxLength={120} />
          </div>
          <div>
          ...
          <div>
            ...
            <input type="datetime-local" min={minExpires} value={expires} onChange={e => setExpires(e.target.value)} className="input" />
          </div>
        </div>
      )}
    </form>
