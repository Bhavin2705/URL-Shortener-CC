...
api.interceptors.response.use(
  ...
  error => {
    ...
    const authInvalid = status === 401 || (status === 403 && message === 'Account is blocked');
    if (authInvalid && typeof window !== 'undefined') {
      window.dispatchEvent(new CustomEvent('snip-auth-invalid', {
