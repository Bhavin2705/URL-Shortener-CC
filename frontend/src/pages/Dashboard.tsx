...
export default function Dashboard() {
  ...
  const load = useCallback((showSpinner = false, showError = true) => {
    if (showSpinner) setLoading(true);
    fetchMyLinks()
      .then(setLinks)
      .catch(() => {
        if (showError) show('Failed to load links', 'error');
      })
      .finally(() => {
  ...
  useEffect(() => {
    ...
    const intervalId = window.setInterval(() => {
      if (document.visibilityState === 'visible') load(false, false);
    }, LINKS_REFRESH_MS);
